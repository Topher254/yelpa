...
const StoryViewer = ({ viewStory, setViewStory }) => {
    ...
    const renderContent = () => {
        switch (viewStory.media_type) {
            ...
            case 'video':
                return (
                    <video onEnded={() => { setViewStory(null) }} src={viewStory.media_url} controls autoPlay className='max-w-full object-contain max-h-screen' />
                )
    ...
    return (
        <div className='fixed inset-0 h-screen bg-black bg-opacity-90 z-110 flex items-center justify-center'
            ...
            <div className='absolute top-4 left-4 flex items-center space-x-3 p-2 px-4 sm:p-4 sm:px-8 backdrop-blur-2xl rounded bg-black/50'>
                <img src={viewStory.user?.profile_picture} className='size-7 rounded-fu;; sm:size-8 rounded-full object-cover border border-white' />
            ...
            <button
                onClick={handleClose}
                className='absolute top-4 right-4 text-white text-3xl font-bold focus:outline-none'><X className='w-8 h-8 hover:scale-110 transition cursor-pointer' /></button>

            {/* content froms troty--we warp it */}
