            ...
            <div className='flex flex-col max-h-56 overflow-y-scroll no-scrollbar'>
                {
                    messages.map((message, index) => (
                        <Link to={`/messages/${message.from_user_id._id}`} key={index} className='flex items-start gap-2 py-2 hover:bg-slate-200'>
                            ...
                            <div className='w-full'>
                                ...
                                <div className='flex justify-between'>
                                    ...
                                    {!message.seen && <p className='bg-purple-500 text-white w-4 h-4 flex items-center justify-center rounded-full text[10px]'>1</p>}
                                </div>
                            </div>
                        </Link>
                    ))
